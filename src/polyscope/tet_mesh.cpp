...
TetMesh::TetMesh(std::string name_, std::vector<glm::vec3> vertices_,
    ...
    : QuantityStructure<TetMesh>(name_), vertices(vertices_), name(name_) {
    ...
    for (size_t i = 0; i < tets_.size(); ++i) {
        std::array<size_t, 4> t{tets_[i][0], tets_[i][1], tets_[i][2],
                                tets_[i][3]};
        tets.emplace_back(t);
...
TetMesh::TetMesh(std::string name_, std::vector<glm::vec3> vertices_,
                 std::vector<std::vector<size_t>> tets_,
                 std::vector<int> tetNeighbors_)
    : QuantityStructure<TetMesh>(name_), vertices(vertices_),
      tetNeighbors(tetNeighbors_), name(name_) {
    // In order Opp0, Opp1, Opp2, Opp3
    localFaces.emplace_back(std::array<size_t, 3>{2, 1, 3});
    localFaces.emplace_back(std::array<size_t, 3>{0, 2, 3});
    localFaces.emplace_back(std::array<size_t, 3>{0, 3, 1});
...
std::vector<EdgePt> TetMesh::clipFace(const std::array<size_t, 3>& face,
                                      const std::array<double, 3>& fn) {
    int numGeqZero = 0;
...
std::vector<EdgePt> TetMesh::clipTet(const std::array<size_t, 4>& tet,
                                     const std::array<double, 4>& fn) {
    std::vector<EdgePt> pts;
    ...
    for (size_t i = 0; i < 4; i++)
        for (size_t j = i + 1; j < 4; j++) {
            if (fn[i] * fn[j] < 0.) {
                ...
                q.emplace_back(t * vertices[tet[j]] +
                               (1.f - t) * vertices[tet[i]]);
...
void TetMesh::precomputeVisibleGeometry() {
    ...
    if (sliceThroughTets) {
        ...
        for (size_t bF : boundaryFaces) {
            std::array<double, 3> fn{distFn(faces[bF][0]), distFn(faces[bF][1]),
                                     distFn(faces[bF][2])};
        ...
        for (std::array<size_t, 4> t : tets) {
            std::array<double, 4> fn{distFn(t[0]), distFn(t[1]), distFn(t[2]),
                                     distFn(t[3])};
        ...
        for (size_t iT = 0; iT < tets.size(); ++iT) {
            if (glm::dot(tetCenters[iT], sliceNormal) <= sliceDist) {
                ...
                for (size_t iF = 4 * iT; iF < 4 * iT + 4; ++iF) {
                    std::vector<EdgePt> face;
                    for (size_t iV : faces[iF]) {
                        face.emplace_back(EdgePt{1, iV, iV});
                    }
                    visibleFaces.emplace_back(face);
                    visibleFaceNormals.emplace_back(faceNormals[iF]);
                }
            }
        }
    }
...
void TetMesh::sliceTet(
    const std::array<glm::vec3, 4>& p,         // in: tet vertices
    ...
    std::vector<double>& T)                    // out: intersection times
{
    ...
    if (q.size() == 4 && dot(cross(q[1] - q[0], q[2] - q[0]),
...
void TetMesh::fillGeometryBuffersPick(gl::GLProgram& p) {
    ...
    for (size_t iF = 0; iF < visibleFaces.size(); ++iF) {
        ...
        for (size_t j = 1; (j + 1) < D; j++) {
            ...
            std::array<size_t, 3> vertexInds = {face[0].src, face[1].src,
                                                face[2].src};
