...
void TetMesh::buildCustomUI(){
 // Print stats
  ...
  ImGui::Text("#verts: %lld  #faces: %lld\n#tets: %lld", nVertsL, nFacesL, nTetsL);

  { // colors
    ImGui::ColorEdit3("Color", (float*)&tetColor, ImGuiColorEditFlags_NoInputs);
    ImGui::SameLine();
  ...
  { // Edge width
    ...
    if(ImGui::SliderFloat("Plane theta", &sliceTheta, 0., 2. * PI, "%.5f", 2.)) {
        sliceNormal = glm::vec3{
            cos(sliceTheta) * cos(slicePhi),
            sin(slicePhi),
            sin(sliceTheta) * cos(slicePhi)
        };
        refillBuffers();
    }
    if(ImGui::SliderFloat("Plane phi", &slicePhi, 0., 2. * PI, "%.5f", 2.)) {
        sliceNormal = glm::vec3{
            cos(sliceTheta) * cos(slicePhi),
            sin(slicePhi),
            sin(sliceTheta) * cos(slicePhi)
