...
TetVectorQuantity::TetVectorQuantity(std::string name, TetMesh& mesh_, MeshElement definedOn_,
                                             VectorType vectorType_)
    : Quantity<TetMesh>(name, mesh_), vectorType(vectorType_), definedOn(definedOn_) {}
...
void TetVectorQuantity::prepareProgram() {
  ...
  for (size_t iT = 0; iT < parent.tets.size(); iT++) {
    if (glm::dot(parent.tetCenters[iT], parent.sliceNormal) > parent.sliceDist) continue;
...
TetVertexVectorQuantity::TetVertexVectorQuantity(std::string name, std::vector<glm::vec3> vectors_,
                                                         TetMesh& mesh_, VectorType vectorType_)
...
TetFaceVectorQuantity::TetFaceVectorQuantity(std::string name, std::vector<glm::vec3> vectors_,
                                                     TetMesh& mesh_, VectorType vectorType_)
